...
def gauss2d(xdata_tuple, amplitude, xo, yo, sigma_x, sigma_y, theta, offset):
    (x,y) = xdata_tuple
    xo = float(xo)
    yo = float(yo)    
...
#######################################################################################
def curvestat1(th,r,bkg):  
      # th - argument
      # r - data
      # bkg -background
      # old procedure, left for compatibility with previous data analyses
      # fails for completely/perfectly symmetric curve
...
def curvestat(th,r,bkg):
      ...
      # r - data
      # bkg -background
      # resolved/improved ambiguity for completely/perfectly symmetric curve
      ...
      if N1 !=N2:
        ...
        for x in rr:              
            if x<half and x0>half:
                i_1=rr_l.index(x0); i_2=rr_l.index(x)
                #print('i_1 = ',i_1)
                #print('i_2 = ',i_2)
                r1=rr[i_1]; r2=rr[i_2]
                th1=thr[i_1]; th2=thr[i_2]
                der2=(r2-r1)/(th2-th1) # linear interpolation: r=der2*(th-th1)+r1                              
                th_pos=th1+(half-r1)/der2
            x0=x 
        #                
