...
def expFh_Be(h,k,l):
      ...
      # 2 atoms per cell
      # M = 9.01 g/mol - molar mass
      # Na=6.022e23 Avogadro's number
      # a=2.2858
      # c=3.5842
      # rho = 1.848 g/cm3
      # N_per_cell = rho*V/(M/Na) = 2
      
      H=2.0*pi*array([h,k,l])
      
      r=[[]]*2
      r[0]=array([1.0/3.0,2.0/3.0,3.0/4.0])
      r[1]=array([2.0/3.0,1.0/3.0,1.0/4.0])
...
def expFh_SiC6H(elem,h,k,l):   #Bauer AC 2001  +1 model agrees with the refinement
    ...
    return Fh
#------------------------------------------------------------------------------------
def expFh_SiO2(elem,h,k,l):   
    
    # atom positions from Kihara EurJMin90
    # these are original Wyckoff positions for left-hand screw structure in right-handed
    # coordinate system 
    H = 2.0*pi*array([h,k,l]) 
    #Wyckoff's from Kihara 
    u0 = 0.4697
    x0 = 0.4132
    y0 = 0.2673
    z0 = 0.1189   
    #Wyckoff's from Wyckoff
    #u0 = 0.465
    #x0 = 0.415 
    #y0 = 0.272
    #z0 = 0.120 
    if elem=='Si':    # Wyckoff posiitons from www.cryst.ehu.es
      r=[[]]*3
      # Wyckoff's
      r[0] = array([u0,0.0,0.0])           #1st Wyckoff's position
      r[1] = array([-u0,-u0,1.0/3.0])      #2nd Wyckoff's position
      r[2] = array([0.0,u0,2.0/3.0])       #3rd Wyckoff's position      
    elif elem=='O':    
      r=[[]]*6
      # Wyckoff's
      r[0] = array([x0,y0,z0])             #1st Wyckoff's position
      r[1] = array([y0-x0,-x0,z0+1.0/3.0]) #2nd Wyckoff's position
      r[2] = array([-y0,x0-y0,z0+2.0/3.0]) #3rd Wyckoff's position
      r[3] = array([x0-y0,-y0,-z0])         #4th Wyckoff's position 
      r[4] = array([y0,x0,2.0/3.0-z0])     #5th Wyckoff's position
      r[5] = array([-x0,y0-x0,1.0/3.0-z0]) #6th Wyckoff's position
